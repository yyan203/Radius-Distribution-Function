...
int main(int argc, char **argv)
{
  ...
  // Provide the correct syntax of this analysis code
    printf("Correct syntax: md.analysis.cxx inputfile outputfile inputparameter(atom id that you want to print its xyz) cellsize speciesA[type] speciesB[type]\n");
    exit(0);
  }
  else {
    ...
    sscanf(argv[5], "%d", &speciesA);
    sscanf(argv[6], "%d", &speciesB);
  }
...
printf("Initialization of myatom  has been successfully finished! %d \n",i);
	...
	myatom[i]->type = type;
      ...
      if(type==speciesB) {nmols_speciesB++;}
	myatom[i]->x[0] = ix;
	myatom[i]->x[1] = iy;
	myatom[i]->x[2] = iz;
       }
...
//printf("comparei:%d  comparej:%d\n",comparei,comparej);
					...
					zi=myatom[comparei]->x[2];zj=myatom[comparej]->x[2];
                                        ...
                                        while (dz<-lz/2.0){dz=dz+lz;}
                                        dist=sqrt(dx*dx+dy*dy+dz*dz); //printf("Done 3\n");
                                        index=int(dist/dr); //printf("index=%d\n",index);
                                        if(index<=nbins){bin[index]++;//printf("bin%d=%d\n",index,bin[index]);
					}
					}
	              			}	
					} 
